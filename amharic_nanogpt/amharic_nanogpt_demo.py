...
def generate(query, max_new_tokens):
    model = torch.load("D:\Projects\\amharic_nanogpt\model.pth")

    model.eval()

    input_ids = torch.tensor([encode(query)], dtype=torch.long, device=device)
...
print(generate(query="የፕሪምየር ሊጉ ጨዋታዎችን", max_new_tokens=800))
